...
void d_print_hostinfo(struct in_addr addr, const struct nb_host_info *hostinfo)
{
  ...
  if (hostinfo->header)
  {
    printf("Transaction ID: 0x%04x (%d)\n", hostinfo->header->transaction_id, hostinfo->header->transaction_id);
    printf("Flags: 0x%04x (%d)\n", hostinfo->header->flags, hostinfo->header->flags);
    printf("Question count: 0x%04x (%d)\n", hostinfo->header->question_count, hostinfo->header->question_count);
    printf("Answer count: 0x%04x (%d)\n", hostinfo->header->answer_count, hostinfo->header->answer_count);
    printf("Name service count: 0x%04x (%d)\n", hostinfo->header->name_service_count, hostinfo->header->name_service_count);
    printf("Additional record count: 0x%04x (%d)\n", hostinfo->header->additional_record_count, hostinfo->header->additional_record_count);
    printf("Question name: %s\n", hostinfo->header->question_name);
    printf("Question type: 0x%04x (%d)\n", hostinfo->header->question_type, hostinfo->header->question_type);
    printf("Question class: 0x%04x (%d)\n", hostinfo->header->question_class, hostinfo->header->question_class);
    printf("Time to live: 0x%08x (%d)\n", hostinfo->header->ttl, hostinfo->header->ttl);
    printf("Rdata length: 0x%04x (%d)\n", hostinfo->header->rdata_length, hostinfo->header->rdata_length);
    printf("Number of names: 0x%02x (%d)\n", hostinfo->header->number_of_names, hostinfo->header->number_of_names);
  };
  ...
  if (hostinfo->footer)
  {
    ...
    printf("Version major: 0x%02x (%d)\n", hostinfo->footer->version_major, hostinfo->footer->version_major);
    printf("Version minor: 0x%02x (%d)\n", hostinfo->footer->version_minor, hostinfo->footer->version_minor);
    printf("Duration: 0x%04x (%d)\n", hostinfo->footer->duration, hostinfo->footer->duration);
    printf("FRMRs Received: 0x%04x (%d)\n", hostinfo->footer->frmps_received, hostinfo->footer->frmps_received);
    printf("FRMRs Transmitted: 0x%04x (%d)\n", hostinfo->footer->frmps_transmitted, hostinfo->footer->frmps_transmitted);
    printf("IFrame Receive errors: 0x%04x (%d)\n", hostinfo->footer->iframe_receive_errors, hostinfo->footer->iframe_receive_errors);
    printf("Transmit aborts: 0x%04x (%d)\n", hostinfo->footer->transmit_aborts, hostinfo->footer->transmit_aborts);
    printf("Transmitted: 0x%08x (%d)\n", hostinfo->footer->transmitted, hostinfo->footer->transmitted);
    printf("Received: 0x%0x8 (%d)\n", hostinfo->footer->received, hostinfo->footer->received);
    printf("IFrame transmit errors: 0x%04x (%d)\n", hostinfo->footer->iframe_transmit_errors, hostinfo->footer->iframe_transmit_errors);
    printf("No receive buffers: 0x%04x (%d)\n", hostinfo->footer->no_receive_buffer, hostinfo->footer->no_receive_buffer);
    printf("tl timeouts: 0x%04x (%d)\n", hostinfo->footer->tl_timeouts, hostinfo->footer->tl_timeouts);
    printf("ti timeouts: 0x%04x (%d)\n", hostinfo->footer->ti_timeouts, hostinfo->footer->ti_timeouts);
    printf("Free NCBS: 0x%04x (%d)\n", hostinfo->footer->free_ncbs, hostinfo->footer->free_ncbs);
    printf("NCBS: 0x%04x (%d)\n", hostinfo->footer->ncbs, hostinfo->footer->ncbs);
    printf("Max NCBS: 0x%04x (%d)\n", hostinfo->footer->max_ncbs, hostinfo->footer->max_ncbs);
    printf("No transmit buffers: 0x%04x (%d)\n", hostinfo->footer->no_transmit_buffers, hostinfo->footer->no_transmit_buffers);
    printf("Max datagram: 0x%04x (%d)\n", hostinfo->footer->max_datagram, hostinfo->footer->max_datagram);
    printf("Pending sessions: 0x%04x (%d)\n", hostinfo->footer->pending_sessions, hostinfo->footer->pending_sessions);
    printf("Max sessions: 0x%04x (%d)\n", hostinfo->footer->max_sessions, hostinfo->footer->max_sessions);
    printf("Packet sessions: 0x%04x (%d)\n", hostinfo->footer->packet_sessions, hostinfo->footer->packet_sessions);
  };
};
...
int main(int argc, char *argv[])
{
  ...
  if (!(quiet || sf || lmhosts || etc_hosts || arrayType))
  ...
  if (!(quiet || verbose || dump || sf || lmhosts || etc_hosts || arrayType))
    print_header();
  if (arrayType)
    // printf("new Array\n(\n");

    if (seperate_segment_scan == 1)
      retransmits = 2;
  for (i = 0; i <= retransmits; i++)
  {

    if (seperate_segment_scan == 1 && i == 0)
    {
      range.start_ip = (range.start_ip & 0xffffff00) | 1;
      range.end_ip = (range.end_ip & 0xffffff00) | 84;
    }
    if (seperate_segment_scan == 1 && i == 1)
    {
      range.start_ip = (range.start_ip & 0xffffff00) | 85;
      range.end_ip = (range.end_ip & 0xffffff00) | 168;
    }
    if (seperate_segment_scan == 1 && i == 2)
    {
      range.start_ip = (range.start_ip & 0xffffff00) | 169;
      range.end_ip = (range.end_ip & 0xffffff00) | 254;
    }
    ...
    while ((select(sock + 1, fdsr, fdsw, NULL, &select_timeout)) > 0)
    {
      if (FD_ISSET(sock, fdsr))
      {
        if ((size = recvfrom(sock, buff, BUFFSIZE, 0,
                             (struct sockaddr *)&dest_sockaddr, (unsigned int *)&addr_size)) <= 0)
        {
