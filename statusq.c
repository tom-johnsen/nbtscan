// This file contains a portion of code from the Samba package, which contains the following license
...
struct nb_host_info *parse_response(char *buff, int buffsize)
{
	...
	if ((response_header = malloc(sizeof(nbname_response_header_t))) == NULL)
		return NULL;
	...
	/* Start with header */
	if (offset + sizeof(response_header->transaction_id) >= buffsize)
		goto broken_packet;
	...
	if (offset + sizeof(response_footer->version_major) >= buffsize)
		goto broken_packet;
	...
	if (offset + sizeof(response_footer->version_minor) >= buffsize)
		goto broken_packet;
